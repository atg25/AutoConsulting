# Cloudflare Worker Dashboard Handoff

## Emergency Paste-Safe Path

If Cloudflare still shows syntax errors, paste from `WORKER_DASHBOARD_INDEX.js` (plain JS file) instead of copying from this markdown code block.

Do NOT paste `worker/src/index.js` into Cloudflare dashboard because that file imports `./foundation.js` and will fail with module resolution errors in single-file mode.

That file is syntax-validated with `node --check`.

Paste the full script below into your Cloudflare Worker `index.js` editor.

- Repo is hardcoded to `atg25/ai-consulting-portfolio`
- Includes one-time `POST /setup`
- Includes hardened `POST /chat`
- Returns clean `401` and `409` messages for mobile UI
